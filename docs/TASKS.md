...
### 2026-09-11 — Doc-audit remediation
- [x] **P2 — Audit softcontrast + focusbeacon internal docs against shipped code** — pro (low) · `surgical-patch`. Audited `softcontrast` and `focusbeacon` READMEs and `IMPLEMENTATION_PLAN.md` files for unverifiable claim-vs-code wording; softened or removed claims and corrected the per-domain storage description.
- [x] **P2 — Resolve remaining unverified README claims** — pro (low) · `surgical-patch`. Removed chromacalm's empty "Screenshots" placeholder; softened terminal-a11y's amber palette and WCAG2ICT wording. Support: `no-ai-slop`.
- [x] **P2 — Reconcile AGENTS.md §4 with available tooling** — pro (low) · `writing-for-agents`. Replaced the Scrapling MCP mandate with the actual available tooling (`gh` CLI, `curl`, `mcp__fetch__fetch`). Merged via `markkirby125/vision-apps` PR #2.
...
### 2026-09-11 — GlareMap + TemporalSafe build and deploy
- [x] **P2 — Build GlareMap** — pro (high) · `javascript-pro` (support: `better-accessibility`, `no-ai-slop`). Planning package ready: `planning/glaremap/IMPLEMENTATION_PLAN.md` + `research/glaremap-research.md`. Hybrid lab + bookmarklet spatial glare heatmap; revisions R1–R3/S5 are mandatory. Phased model/effort in `## Phased Implementation Plans` below. **Deployed 2026-09-11**: repo `markkirby125/glaremap`, CI green (test + pages), live at `https://markkirby125.github.io/glaremap/`. Remaining verification: manual bookmarklet smoke on 3 real sites (incl. one strict-CSP) + keyboard/screen-reader walk.
- [x] **P2 — Build TemporalSafe** — pro (high) · `javascript-pro` (support: `better-accessibility`, `no-ai-slop`). Planning package ready: `planning/temporalsafe/IMPLEMENTATION_PLAN.md` + `research/temporalsafe-research.md`. Page-level flicker/flash reducer (userscript + demo page); revisions R4–R6 mandatory. Phased model/effort in `## Phased Implementation Plans` below. **Deployed 2026-09-11**: repo `markkirby125/temporalsafe`, CI green (test + pages), live at `https://markkirby125.github.io/temporalsafe/`. Remaining verification: manual SPA/news-site smoke + keyboard/screen-reader walk.
...
- [x] **P2 — Brainstorming design pass for all six ideas** — pro (high) · `brainstorming`. Understanding lock confirmed by user for all six at buildable-spec depth. Produced `research/browser-vision-app-ideas-design.md`: per-idea design approaches (recommended + rejected with reasons), architecture, components, data flow, edge cases, tool accessibility, testing strategy, risks, and a 9-entry decision log. Next gates before any implementation: `multi-agent-brainstorming` peer review on the ⭐ specs, then user picks build order.
- [x] **P2 — multi-agent peer review of the ⭐ specs** — pro (high) · `multi-agent-brainstorming`. Ran Skeptic → Constraint Guardian → User Advocate → Arbiter on GlareMap, TemporalSafe, Keratoscope. Final disposition: **REVISE** for all three (none rejected, none approved as-is). Mandatory revisions recorded in `research/browser-vision-app-ideas-design.md` (Peer Review section + review decision log R1–R9): GlareMap input-safe mask/accuracy/perf budget, TemporalSafe gentle default/scope honesty/throttling, Keratoscope no-guarantee wording/anchored calibration/single-mode v1/safety prompt. Implementation may begin only after these revisions are folded into per-idea specs.
- [x] **P2 — Fold peer-review revisions R1–R9 into the ⭐ specs** — pro (low) · `surgical-patch`. Planning-only (no code). Updated GlareMap, TemporalSafe and Keratoscope spec sections in `research/browser-vision-app-ideas-design.md` to incorporate every accepted objection: GlareMap (≤2,000-element cap, ~96×54 grid, `pointer-events: none` mask skipping interactive elements, unmeasured image/video regions, off switch + plain-language label, CSP fallback), TemporalSafe (Reduced default / Photosensitive opt-in, per-element undo + site allowlist, no layout removal, motion-triggered 500ms sampling + idle sleep, cross-origin iframe/rAF scope statement), Keratoscope (no-guarantee wording, anchored calibration <2 min, mandatory preview, single light-mode v1, one default userstyle export, safety prompt). "Next steps" section re-gated: build-order selection + per-project planning artifacts before any code. No implementation tonight.

### 2026-09-10 — README copy & AISEO SOP comparisons
- [x] **P2 — Compare all 5 repo READMEs vs SOP_Direct_Response_Copywriting.md** — pro (low) · `no-ai-slop`. Audited `README.md`, `chromacalm/README.md`, `softcontrast/README.md`, `focusbeacon/README.md`, `terminal-a11y/README.md` (+ `llms.txt`) against the Sabri Suby/King Kong 17-step framework. Verdict: pain-agitation and unique-mechanism steps are already strong (`The Science` / `The Problem` / `Clinical Motivation` sections), but the conversion layer is absent everywhere — no audience eyebrow, no benefit sub-headline (except SoftContrast's), features listed as raw features instead of FAS (Feature→Advantage→Benefit), zero social proof, no embedded-command CTA. `chromacalm/README.md` still contains the SOP's banned token "Furthermore". Remediation applied 2026-09-10 (eyebrow, sub-headline, FAS, CTA, banned-token removal).
- [x] **P2 — Compare all 5 repo READMEs vs SOP_AISEO.md** — pro (low) · `ai-seo`. Audited the same six files against the README-applicable modules of the 4,136-line AISEO SOP (Modules 1, 2, 7.4, 8, 9 and the Module 10 supplements; Modules 3/5/6 are local-GBP/ads/Discover and N/A for GitHub READMEs). Verdict: BLUF front-loading, heading order, bullet/table structure, compactness and standards citations pass; gaps are no `llms.txt` in the four project repos, no negative-qualification trust content (§2.15), no freshness timestamps, thin entity/author attribution (§2.26), vague section headings like `The Science` (§2.7/§2.20), and the verbatim copy-pasted "Part of the Vision Apps toolkit" block across all four project READMEs — a template-footprint/sameness risk per §2.32. Remediation applied 2026-09-10 (llms.txt manifests, trust content, dates, title-grade headings, author attribution, de-duplicated toolkit intro).
